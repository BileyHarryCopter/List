...
int LogToPhys(List_t *list, int log_pos) {
  assert(list);
  logassert(log_pos < list->insertion, Logfile, __PRETTY_FUNCTION__,
            FIND_UNEXISTED, list);
...
int PhysToLog(List_t *list, int phys_pos) {
  assert(list);
  logassert(list->prev[phys_pos] != -1, Logfile, __PRETTY_FUNCTION__,
            FIND_UNEXISTED, list);
...
int ListResup(List_t *list) {
  ...
  list->data = (data_t *)realloc(list->data,
                                 INCR_KOEF * (list->capacity) * sizeof(data_t));
  list->next =
      (int *)realloc(list->next, INCR_KOEF * (list->capacity) * sizeof(int));
  list->prev =
      (int *)realloc(list->prev, INCR_KOEF * (list->capacity) * sizeof(int));
...
int ListInsrt(List_t *list, int mode, int insrt_ptr, data_t insrt_val) {
  assert(list);
  logassert(insrt_ptr < list->capacity && list->prev[insrt_ptr] != -1, Logfile,
            __PRETTY_FUNCTION__, INSERT_ERROR, list);
...
int ListDelete(List_t *list, int del_ptr) {
  assert(list);
  logassert(del_ptr < list->capacity && list->prev[del_ptr] != -1, Logfile,
            __PRETTY_FUNCTION__, DELETE_UNEXISTED, list);
  logassert(list->insertion != 0, Logfile, __PRETTY_FUNCTION__, DELETE_EMPTY,
            list);
  logassert(del_ptr != 0, Logfile, __PRETTY_FUNCTION__, DELETE_FICT, list);
...
int ListLinearisation(List_t *list) {
  assert(list);
  logassert(list->insertion != 0, Logfile, __PRETTY_FUNCTION__, LINEAR_ERROR,
            list);
